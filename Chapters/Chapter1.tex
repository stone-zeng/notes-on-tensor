...
\subsection{向量的分量}
	...
	\begin{equation}
		\V{\xi}=\ipb{\V{\xi}}{\V{g}^k}{\Rm}\,\V{g}_k
		=\xi^k \V{g}_k \comma
	\end{equation}
	也可以用逆变基表示：
	\begin{equation}
		\V{\xi}=\ipb{\V{\xi}}{\V{g}_k}{\Rm}\,\V{g}^k
		=\xi_k \V{g}^k \comma
	\end{equation}
...
\subsection{张量分量之间的关系}
	...
	\begin{braceEq}
		\xi^i &=\ipb{\V{\xi}}{\V{g}^i}{\Rm}
		=\ipb{\V{\xi}}{g^{ik}\V{g}_k}{\Rm}
		=g^{ik}\ipb{\V{\xi}}{\V{g}_k}{\Rm}
		=g^{ik}\xi_k \comma \\
		\xi_i &=\ipb{\V{\xi}}{\V{g}_i}{\Rm}
		=\ipb{\V{\xi}}{g_{ik}\V{g}^k}{\Rm}
		=g_{ik}\ipb{\V{\xi}}{\V{g}^k}{\Rm}
		=g_{ik}\xi^k \fullstop
	\end{braceEq}
