...
可以用该点处张量场的\emphA{梯度}（即 $\T{\Phi}\tp\T{\nabla}$）
...
\end{equation}
“$\T{\nabla}$”在右边，故称之为\emphA{右梯度}（简称\emphA{梯度}）。
相应地，自然会有\emphA{左梯度}：
\begin{equation}
...
\begin{align}
	...
	&=\nabla_\mu g_{ij}\,\V{g}^i\tp\V{g}^j \comma
\end{align}
式中，协变导数定义为
\begin{equation}
	\nabla_\mu g_{ij} \defeq \pdv{g_{ij}}{x^\mu}
...
\begin{equation}
	\nabla_\mu g_{ij} = \nabla_\mu g^{ij}
	=\nabla_\mu \KroneckerDelta{i}{j} = 0 \fullstop
\end{equation}
此结论称为\emphA{Ricci引理}。
...
于是便有 $\nabla_\mu \LeviCivita{^i_j^k}=0$。进而
\begin{equation}
	\pdv{\EdTensor}{x^\mu} (\V{x})=\T{0}\in\Tensors[\realR^3]{3}
	\fullstop
\end{equation}

和之前的 Ricci 引理类似，Eddington 张量其他形式分量的偏导数，
如 $\nabla_\mu\LeviCivita{_{ijk}}$、
$\nabla_\mu\LeviCivita{^{ijk}}$ 等，也都等于零。
\end{myProof}