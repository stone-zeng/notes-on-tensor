...
\begin{align}
	&\alspace\tensor{\Phi}{^i_j^k} (\V{x}) \,\,
		\pdv{\V{g}_i}{x^\mu} (\V{x})
		\tp\V{g}^j(\V{x})\tp\V{g}_k(\V{x}) \notag \\
	&=\ChristoffelB{\mu}{i}{s} \tensor{\Phi}{^i_j^k}(\V{x}) \,\,
		\V{g}_s(\V{x})\tp\V{g}^j(\V{x})\tp\V{g}_k(\V{x}) \notag
	\intertext{$i$ 和 $s$ 都是哑标，不妨进行一下交换：}
	&=\ChristoffelB{\mu}{s}{i} \tensor{\Phi}{^s_j^k}(\V{x}) \,\,
...
\begin{align}
	...
	\lim_{\lambda\to 0} \frac{\T{\Phi}\qty(\V{x}+\lambda\,\V{e}_\mu)
		-\T{\Phi}(\V{x})} {\lambda} \notag \\
	&=\qty[\qty\Bigg(\pdv{\tensor{\Phi}{^i_j^k}}{x^\mu}
	...
	&\eqcolon \nabla_\mu \tensor{\Phi}{^i_j^k} (\V{x}) \,\,
		\V{g}_i(\V{x})\tp\V{g}^j(\V{x})\tp\V{g}_k(\V{x}) \comma
\end{align}
式中，我们称 $\nabla_\mu \tensor{\Phi}{^i_j^k}(\V{x})\in\realR$
为张量分量 $\tensor{\Phi}{^i_j^k}(\V{x})$ 相对于 $x^\mu$
的\emphA{协变导数}，其定义为：
\begin{equation}
	\nabla_\mu \tensor{\Phi}{^i_j^k} (\V{x}) \defeq
	\pdv{\tensor{\Phi}{^i_j^k}}{x^\mu} (\V{x})
	+\ChristoffelB{\mu}{s}{i} \tensor{\Phi}{^s_j^k}(\V{x})
	-\ChristoffelB{\mu}{j}{s} \tensor{\Phi}{^i_s^k}(\V{x})
	+\ChristoffelB{\mu}{s}{k} \tensor{\Phi}{^i_j^s}(\V{x})
	\fullstop
\end{equation}
