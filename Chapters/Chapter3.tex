...
\begin{equation}
	\T{\EddT}=\EddT_{ijk}\,\V{g}^i\tp\V{g}^j\tp\V{g}^k
	\in\Tensors{3} \comma
\end{equation}
式中的$\EddT_{ijk}=\det[\V{g}_i,\,\V{g}_j,\,\V{g}_k]$。
和之前一样，仍是利用指标升降来获得等价定义：
\begin{align}
	\T{\EddT}&=\EddT_{ijk}\,\V{g}^i\tp\V{g}^j\tp\V{g}^k \notag \\
	&=\EddT_{ijk}\,\V{g}^i\tp\qty(g^{jl}\,\V{g}_l)
		\tp\V{g}^k \notag \\
	&=\EddT_{ijk}\,g^{jl}\, \V{g}^i\tp\V{g}_l\tp\V{g}^k \notag
	\intertext{根据张量分量之间的关系
		（回顾 \ref{subsec:张量分量之间的关系}~小节），我们有}
	&=\tensor{\EddT}{_i^l_k}\,\V{g}^i\tp\V{g}_l\tp\V{g}^k \fullstop
\end{align}
当然，这里的 $\tensor{\EddT}{_i^l_k}$ 只是一个形式。
