...
\end{equation}
则称向量值映射 $\V{X}(\V{x})$ 在 $\V{x}_0$ 点\emphA{可微}。
...
（$\lambda \neq 0$）。
代入可微性的定义 \eqref{eq:向量值映照可微性的定义}~式，有
\begin{align}
...
\end{equation}
它表示因变量 $\V{\V{X}}\in\Rm$ 作为一个\emphB{整体}，
相对于自变量 $\V{x}\in\Rm$ 第 $i$ 个\emphB{分量} $x^i\in\realR$
的“变化率”，即 $\V{\V{X}}$ 关于 $x^i$（在 $\V{x}_0$ 处）%
