...
%TODO——选择编译
\ifdefined\DEBUG
	\includeonly{Chapters/Surface_Frame}
	\renewcommand{\part}[1]{}
...
\fi
	...
	\part{张量代数}
		% 张量的定义及表示
		\include{Chapters/Tensor_Definition}
		% 张量的运算性质
		\include{Chapters/Tensor_Properties}
	
	\part{体积上的张量场场论}
		% 微分同胚（曲线坐标系）
		\include{Chapters/Diffeomorphism}
		% 张量场可微性
		\include{Chapters/Tensor_Field_Derivative}
		% 非完整基理论
		\include{Chapters/Nonholonomic_Basis_Theory}
		% 曲线上的标架及其运动方程
		\include{Chapters/Curve_Frame}
	
	\part{曲面上的张量场场论}
		% 曲面与曲面标架
		\include{Chapters/Surface_Frame}
